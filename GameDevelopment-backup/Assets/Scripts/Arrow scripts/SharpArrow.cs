...
public class SharpArrow : MonoBehaviour
{
    ...
    // ✅ Kill the player when they touch the arrow
    private void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.CompareTag("Player")) // If the Player collides with the arrow
        {
    ...
    void KillPlayer(GameObject player)
    {
        ...
        if (playerAnim != null)
        {
            playerAnim.Play("PlayerDie"); // ✅ Play death animation
        }

        if (playerScript != null)
        {
            playerScript.enabled = false; // Disable player movement
        }
